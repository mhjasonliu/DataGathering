...
/**
 * Created by Y.Misal on 7/17/2017.
 */

public class GyroWriteDataThread extends AsyncTask<Void, Void, Void> implements Thread.UncaughtExceptionHandler {
    private static final String TAG = "GyroWriteDataThread";

    volatile boolean mRunning = true;
    volatile boolean mWriting = true;
    //set mRunning to false when terminating appln
    public Queue<DataAccumulator> mQueue;
    private final Object mGyroObj;
    private Context mContext;

    public GyroWriteDataThread(Context context) {
        mContext = context;
    ...
    public void SaveToFile(DataAccumulator acc) {
        synchronized (mGyroObj) {
            try {
                mQueue.add(acc);
                Log.v(TAG, "queue size after adding " + mQueue.size() + " recently added "+acc.type+" & size "+acc.getCount());
                ...
                if(mQueue.size() > 0) {
                    ...
                    if (mWriting) {
                        try {
                            mWriting = false;
                            first = mQueue.remove();
                            Log.v(TAG, "mQueue.remove() " + mQueue.size() + " recently removed " + first.type);
                            //process first.
                            //save on 10 sec delay.
                            /*try {
                                Thread.sleep(10000);
