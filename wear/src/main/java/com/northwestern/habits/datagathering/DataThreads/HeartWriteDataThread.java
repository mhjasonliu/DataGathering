...
/**
 * Created by Y.Misal on 7/18/2017.
 */

public class HeartWriteDataThread extends AsyncTask<Void, Void, Void> implements Thread.UncaughtExceptionHandler {
    private static final String TAG = "HeartWriteDataThread";
    ...
    private final Object mHeartObj;
    private Context mContext;

    public HeartWriteDataThread(Context context) {
        mContext = context;
        mQueue   = new LinkedList<DataAccumulator>();
        mHeartObj = new Object();
    }

    public void SaveToFile(DataAccumulator acc) {
        synchronized (mHeartObj) {
            try {
                mQueue.add(acc);
            ...
            Log.v(TAG, "current queue size " + mQueue.size() + " recently added "+acc.type+" & size "+acc.getCount());
        }
    }
